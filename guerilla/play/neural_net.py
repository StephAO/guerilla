...
class NeuralNet:
    ...
    def __init__(self, load_file=None, hp_load_file=None, seed=None, verbose=True, hp=None):
        ...
        # Bin information -> generated by self.generate_bins()
        if self.hp['CLASSIFIER']:
    ...
    def generate_bins(self, cp_values):
        """
        Generates the class bins for the neural network based on the given CP values and their distribution.
        Tries to create bins of uniform size.
        :param cp_values: [List of Ints] List of CP values from which the bin sizes should be inferred.
        :return: 
        """

        if not self.hp['CLASSIFIER']:
            raise ValueError("Cannot generate bins for non-Classifier Neural Network!")

        cp_count = {}

        # Get the count for each CP value
        for val in cp_values:
            if val not in cp_count:
                cp_count[val] = 0
            cp_count[val] += 1

        bin_size = len(cp_values) / self.hp['NUM_CLASS']
        while True:
            self.class_bins = [dh.LOSE_VALUE]

            curr_bin_count = 0
            all_bin_count = 0
            for i in xrange(dh.LOSE_VALUE, dh.WIN_VALUE + 1):
                # Add the CP values to the current bin (add 0 if no corresponding CP values)
                curr_bin_count += cp_count.get(i, 0)
                all_bin_count += cp_count.get(i, 0)

                # start a new bin
                if curr_bin_count > bin_size:
                    self.class_bins.append(i + 1)
                    curr_bin_count = 0

                # When out of bins then place all remaining elements in last bin
                #   Note: Number of bins = len(class_bins) - 1
                if len(self.class_bins) == self.hp['NUM_CLASS']:
                    self.class_bins.append(dh.WIN_VALUE + 1)
                    break
            else:
                # Not enough bins, decrease bin size by 5% of uniform bin size
                bin_size -= (len(cp_values) / self.hp['NUM_CLASS']) * 0.05

                print "Generated {} bins (goal is {} bins), reduced bin size to {} and regenerating...".format(
                    len(self.class_bins) - 1, self.hp['NUM_CLASS'], bin_size)
    ...
    def model(self):
        ...
        if self.hp['CLASSIFIER']:
            self.logits = self.fc_layer(fc2, self.hp['NUM_HIDDEN'] / 2, self.hp['NUM_CLASS'], "predicted_value",
                                        activation_fn=None)
